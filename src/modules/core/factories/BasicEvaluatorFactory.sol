...
contract BasicEvaluatorFactory is IEvaluatorFactory {
    ...
    function deployEvaluator(address dac, uint256 id, DealParams calldata params) external returns (address) {
        Milestone[] memory schedule = abi.decode(params.evaluatorConfig, (Milestone[]));

        BasicEvaluator eval = new BasicEvaluator(dac, id, schedule);
